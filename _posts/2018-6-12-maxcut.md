...
$$\mathbb{E} \text{CUT}(G,x) = \frac{1}{4} \sum_{i,j} A_{ij}(1 - \mathbb{E}[x_ix_j]) = \frac{1}{4}\sum_{i,j} A_{ij} = \frac{1}{2} |E|.$$

So we expect to get a cut with size $$\frac{1}{2} |E|$$. But $$|E|$$ is trivially an upper-bound on  $$\text{MAX-CUT}(G)$$, so in this case, our algorithm gives us a solution that is $$\approx 0.5 \text{MAX-CUT}(G)$$.
...
$$\text{SDP(G)} := \text{maximize} \frac{1}{4} \sum_{i,j} A_{ij} (1-\left< X_i,X_j \right>): ||X_i||_2 = 1$$
...
**Lemma.** For $$g\sim\mathcal{N}(0,I_n)$$, any $$u,v\in\mathbb{R}^n$$ with $$||u||_2=||v||_2=1$$:
...
**Proof.** $$\begin{align}\mathbb{E}\left[ \text{sign} \left<g, u \right>\text{sign} \left<g, v \right>\right] = &1 
